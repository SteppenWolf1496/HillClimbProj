...
public class UniversalWheel : MonoBehaviour
{
    ...
    [HideInInspector] public Vector4
        forwardFrictionVars;
    ...
    [HideInInspector] public AnimationCurve
        forwardAnimCurve;
    ...
    {
        get { return wheelDownTravel; }
    }

    public float SidewaySlip
    {
        get { return sidewaySlip; }
    }

    public float ForwardSlip
    {
        get { return forwardSlip; }
    }
   ...
   public virtual void Start()
    ...
    void CalculateSuspensionForces()
    {
        ...
        if (Physics.Raycast(ray, out hit, radius + suspensionTravel, collisionMask))
        {
            ...
            if (affectOtherRigidbodies && hit.transform.GetComponent<Rigidbody>() != null &&
    ...
    void CalculateFrictionForces()
    {
        ...
        sidewayForce = myRight*Mathf.Sign(sidewaySlip)*((sidewayFrictionCurve.GetFriction(sidewaySlip)))*
                       onGroundPressureForce;
        if (maxSideForce > 0)
        {
        ...
        forwardForce = myForward*Mathf.Sign(forwardSlip)*(forwardFrictionCurve.GetFriction(forwardSlip))*
                       onGroundPressureForce;
    ...
    void UpdateVisuals()
    {
        if (fluentWheelMovement)
        {
            ...
            visualWheel.localPosition = Vector3.Lerp(visualWheel.localPosition,
                new Vector3(visualWheel.localPosition.x, -wheelDownTravel, visualWheel.localPosition.z),
                updateCoef*Time.deltaTime);
        }
        else
        {
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawLine(transform.position,
            transform.position - transform.up*((1 - percentOfSpringCompression)*suspensionTravel));
        ...
        {
            pos =
                transform.TransformPoint(suspensionMove +
                                         radius*
                                         new Vector3(0, Mathf.Sin(i/20.0f*Mathf.PI*2.0f),
                                             Mathf.Cos(i/20.0f*Mathf.PI*2.0f)));
